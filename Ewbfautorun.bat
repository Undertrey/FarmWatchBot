...
IF "%lasterror%" NEQ "0" (
	...
	ECHO %lasterror%| findstr.exe /I /R %warningslist% 2>NUL && (
		...
		ECHO                        Miner ran for %hrdiff%:%mediff%:%ssdiff%
		ECHO +================================================================+
		CALL :inform "false" "0" "Current temp: %curtemp%." "2"
		...
		CALL :inform "false" "Current temp: %curtemp%.%%%%0A%%%%0ATemperature limit reached. Fans may be stuck. Attempting to restart computer..." "Temperature limit reached. Fans may be stuck." "2"
		GOTO restart
	)
)
timeout.exe /T 5 /nobreak >NUL
...
IF "%curtemp%" NEQ "unknown" ECHO Current temp: %curtemp%.
IF "%curspeed%" NEQ "unknown" ECHO Current speed: %curspeed%.
ECHO +================================================================+
...
IF "%curspeed%" NEQ "unknown" SET statusmessage=%statusmessage%%%%%0ACurrent speed: %curspeed%
IF "%curtemp%" NEQ "unknown" SET statusmessage=%statusmessage%%%%%0ACurrent temp: %curtemp%
IF "%chatid%" NEQ "0" (
