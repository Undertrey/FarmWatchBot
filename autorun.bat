...
IF EXIST "miner.log" (
	MOVE /Y miner.log Logs\miner_%Y0%.%M0%.%D0%_%H0%.%X0%.%C0%.log 2>NUL 1>&2 && ECHO miner.log renamed and moved to Logs folder. || (
		...
		DEL /Q /F "miner.log" >NUL || (
			...
			IF %EnableTelegramNotifications% EQU 1 powershell.exe -command "(new-object net.webclient).DownloadString('%Web%&chat_id=%ChatId%&text=*%RigName%:* Unable to delete miner.log.')" 2>NUL 1>&2
...
IF NOT EXIST "miner.log" (
	...
	IF %EnableTelegramNotifications% EQU 1 powershell.exe -command "(new-object net.webclient).DownloadString('%Web%&chat_id=%ChatId%&text=*%RigName%:* Error. miner.log is missing.%%0ACheck permissions of this folder. This script requires permission to create files.')" 2>NUL 1>&2
