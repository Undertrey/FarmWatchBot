...
SET Server1BatCommand=%MinerProcess% -zpool eu1-zcash.flypool.org:3333 -zwal t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr178 -zpsw x -i 7 -tstop 95 -logfile miner.log
SET Server2BatCommand=%MinerProcess% -zpool eu1-zcash.flypool.org:3333 -zwal t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr178 -zpsw x -i 7 -tstop 95 -logfile miner.log
SET Server3BatCommand=%MinerProcess% -zpool eu1-zcash.flypool.org:3333 -zwal t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr178 -zpsw x -i 7 -tstop 95 -logfile miner.log
SET Server4BatCommand=%MinerProcess% -zpool eu1-zcash.flypool.org:3333 -zwal t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr178 -zpsw x -i 7 -tstop 95 -logfile miner.log
SET Server5BatCommand=%MinerProcess% -zpool eu1-zcash.flypool.org:3333 -zwal t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr178 -zpsw x -i 7 -tstop 95 -logfile miner.log
...
IF EXIST "config.bat" (
	findstr.exe /C:"%Version%" config.bat >NUL && (
		FOR %%A IN (%~n0.bat) DO IF %%~ZA LSS 49812 EXIT
...
IF !FirstRun! NEQ 0 (
	...
	timeout.exe /T 1 /nobreak >NUL
	FOR /F "tokens=2,5,8,11,14,17,20,23,26,29,32,35,38,41,44 delims==,tCGPUfan% " %%a IN ('findstr.exe /R /C:"GPU.* t=.*C fan=.*" miner.log') DO (
	...
	timeout.exe /T 1 /nobreak >NUL
	FOR /F "tokens=3,6,9,12,15,18,21,24,27,30,33,36,39,42,45 delims=.,H/sGPU " %%a IN ('findstr.exe /R /C:"GPU.* .* H/s.*" miner.log') DO (
				...
				ECHO !LastInternetError!| findstr.exe /I /R %InternetErrorsList% >NUL && (
					...
					ping.exe google.com| find.exe /I "TTL=" >NUL && (
						...
						IF %ServerQueue% EQU 5 (
							...
							>> %MinerBat% ECHO %MinerProcess% -zpool eu1-zcash.flypool.org:3333 -zwal t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr178 -zpsw x -i 7 -tstop 95 -logfile miner.log
							SET ServerQueue=1
						)
						>> %MinerBat% ECHO EXIT
