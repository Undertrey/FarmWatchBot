...
IF %EnableTelegramNotifications% EQU 1 (
	...
	IF %EnableOnlineStatusSend% EQU 1 (
		...
		IF !PTOS! LSS %X2% (
			IF !LastBot! EQU 1 (
				:bot1
				timeout.exe /T 1 /nobreak >NUL && powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot466765661:AAHvbf66WH5JqdD5yQzCh21nR87aV1zik8I/sendMessage?chat_id=-1001121778849&text=,%ChatId%,%RigName%,')" 2>NUL 1>&2 || GOTO bot2
				SET /A PTOS=%X2%+3
				SET LastBot=1
			)
			IF !LastBot! EQU 2 (
				:bot2
				timeout.exe /T 1 /nobreak >NUL && powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot432276393:AAHdnzhf0UCUMJCDT1n2O_FBog7ZI7VbK6I/sendMessage?chat_id=-1001121778849&text=,%ChatId%,%RigName%,')" 2>NUL 1>&2 || GOTO bot3
				SET /A PTOS=%X2%+3
				SET LastBot=2
			)
			IF !LastBot! EQU 3 (
				:bot3
				timeout.exe /T 1 /nobreak >NUL && powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot467076517:AAFXjw7wEmJelLDcgVl7AxMZ5fVxFSr1QTU/sendMessage?chat_id=-1001121778849&text=,%ChatId%,%RigName%,')" 2>NUL 1>&2 || GOTO bot4
				SET /A PTOS=%X2%+3
				SET LastBot=3
			)
			IF !LastBot! EQU 4 (
				:bot4
				timeout.exe /T 1 /nobreak >NUL && powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot469692661:AAFZkmGACu7WJbOfYLXCf-R2dc41-F_pVto/sendMessage?chat_id=-1001121778849&text=,%ChatId%,%RigName%,')" 2>NUL 1>&2 || GOTO bot5
				SET /A PTOS=%X2%+3
				SET LastBot=4
			)
			IF !LastBot! GEQ 5 (
				:bot5
				timeout.exe /T 1 /nobreak >NUL && powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot451219247:AAFESo00xfgBIb2huhx1o7KPtGuS3VgA_fE/sendMessage?chat_id=-1001121778849&text=,%ChatId%,%RigName%,')" 2>NUL 1>&2 || GOTO bot1
				SET /A PTOS=%X2%+3
				SET LastBot=5
			)
		)
			...
			FOR /F "tokens=3,12,16,19 delims=,:}{" %%c IN ("%%b") DO (
				...
				IF !Utdiff! LEQ 80 (
					...
					IF !UpdateId! GTR !OldUpdateId! IF !MessageId! NEQ !OldMessageId! (
						...
						ECHO %%e | findstr.exe /I /R /C:"^restart.*%RigName% .*$" /C:"^restart $" /C:"^restart@EwbfWatchBot.*%RigName% .*$" /C:"^restart@EwbfWatchBot $" >NUL && (
							>> %~n0.log ECHO [%NowDate%][%NowTime%] Command /%%e received.& ECHO [%NowDate%][%NowTime%] Command /%%e received.
						...
						ECHO %%e | findstr.exe /I /R /C:"^startover.*%RigName% .*$" /C:"^startover $" /C:"^startover@EwbfWatchBot.*%RigName% .*$" /C:"^startover@EwbfWatchBot $" >NUL && (
							>> %~n0.log ECHO [%NowDate%][%NowTime%] Command /%%e received.& ECHO [%NowDate%][%NowTime%] Command /%%e received.
						...
						ECHO %%e | findstr.exe /I /R /C:"^online.*%RigName% .*$" /C:"^online $" /C:"^online@EwbfWatchBot.*%RigName% .*$" /C:"^online@EwbfWatchBot $" >NUL && (
							>> %~n0.log ECHO [%NowDate%][%NowTime%] Command /%%e received.& ECHO [%NowDate%][%NowTime%] Command /%%e received.
							powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot%Num%:%prt%-%rtp%%tpr%/sendMessage?parse_mode=markdown&chat_id=%ChatId%&text=*%RigName%* Online, *%t3h%:%t3m%*, *!LastHashrate!*.')" 2>NUL 1>&2 && GOTO check
						)
						ECHO %%e | findstr.exe /I /R /C:"^info.*%RigName% .*$" /C:"^info $" /C:"^info@EwbfWatchBot.*%RigName% .*$" /C:"^info@EwbfWatchBot $" >NUL && (
							>> %~n0.log ECHO [%NowDate%][%NowTime%] Command /%%e received.& ECHO [%NowDate%][%NowTime%] Command /%%e received.
							powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot%Num%:%prt%-%rtp%%tpr%/sendMessage?parse_mode=markdown&chat_id=%ChatId%&text=*%RigName%:* Miner has been running for *%t3h%:%t3m%* - do not worry.%%0AAverage total hashrate: *!SumResult!*.%%0ALast total hashrate: *!LastHashrate!*.%%0ACurrent Speed: !CurrentSpeed!.%%0ACurrent !CurrentTemp!.')" 2>NUL 1>&2 && GOTO check
						)
						ECHO %%e | findstr.exe /I /R /C:"^stop.*%RigName% .*$" /C:"^stop $" /C:"^stop@EwbfWatchBot.*%RigName% .*$" /C:"^stop@EwbfWatchBot $" >NUL && (
							taskkill.exe /F /IM "%MinerProcess%" 2>NUL 1>&2 && powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot%Num%:%prt%-%rtp%%tpr%/sendMessage?parse_mode=markdown&chat_id=%ChatId%&text=*%RigName%:* Process *%MinerProcess%* was successfully *killed* by /stop command. Use /startover command to begin mining again, or restart *autorun.bat* manually.')" 2>NUL 1>&2 && (
								>> %~n0.log ECHO [%NowDate%][%NowTime%] Process %MinerProcess% was successfully killed by /stop command.
								ECHO Process %MinerProcess% was successfully killed by /stop command.
								SET WaitUserInput=1
								GOTO tcomm
							)
						)
						ECHO %%e | findstr.exe /I /R /C:"^profile.*%RigName% .*[1-5] $" /C:"^profile [1-5] $" /C:"^profile@EwbfWatchBot.*%RigName% .*[1-5] $" /C:"^profile@EwbfWatchBot [1-5] $" >NUL && (
							SET ProfileNum=%%e
						...
						ECHO %%e | findstr.exe /I /R /C:"^ping.*%RigName% .*$" /C:"^ping $" /C:"^ping@EwbfWatchBot.*%RigName% .*$" /C:"^ping@EwbfWatchBot $" >NUL && (
							FOR /F "tokens=3 delims= " %%s IN ('findstr.exe /C:"--server" %MinerBat%') DO (
						...
						ECHO %%e | findstr.exe /I /R /C:"^pool.*%RigName% .*$" /C:"^pool $" /C:"^pool@EwbfWatchBot.*%RigName% .*$" /C:"^pool@EwbfWatchBot $" >NUL && (
							FOR /F "tokens=3 delims= " %%s IN ('findstr.exe /C:"--server" %MinerBat%') DO SET CurrServerName=%%s
							...
							>> %~n0.log ECHO [%NowDate%][%NowTime%] Command /%%e received.& ECHO [%NowDate%][%NowTime%] Command /%%e received.
							powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot%Num%:%prt%-%rtp%%tpr%/sendMessage?parse_mode=markdown&chat_id=%ChatId%&text=*%RigName%:* Currently ready for mining on server: !CurrServerName!.%%0A%%0AMain server:%%0A0. !MainServerName!.%%0A%%0AAdditional servers:%%0A1. !Add1ServerName!.%%0A2. !Add2ServerName!.%%0A3. !Add3ServerName!.%%0A4. !Add4ServerName!.')" 2>NUL 1>&2 && GOTO check
						)
						ECHO %%e | findstr /I /R /C:"^server.*%RigName% .*[0-4] $" /C:"^server [0-4] $" /C:"^server@EwbfWatchBot.*%RigName% .*[0-4] $" /C:"^server@EwbfWatchBot [0-4] $" >NUL && (
							SET ServerNum=%%e
							...
							>> %~n0.log ECHO [%NowDate%][%NowTime%] Warning. Pool server was switched by /server command to additional server !ServerNum!.
							ECHO Warning. Pool server was switched by /server command to additional server !ServerNum!.
							GOTO start
						)
					)
				)
