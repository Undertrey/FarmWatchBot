...
SET MinerErrorsList=/C:".*CUDA error.*" /C:".*unknown error.*" /C:".*unresponsive.*" /C:".*was encountered.*" /C:".*cuda.*failed.*" /C:".* [0-5]C .*"
...
IF NOT EXIST "%Logfile%" (
	...
	IF %ChatId% NEQ 0 CALL :telegram "false" "%Logfile% is missing. Ensure *--logfile* option is added to the miners command line. Check permissions of this folder. This script requires permission to create files."
	...
	findstr.exe /R /C:".*--server.*--logfile.*--time.*--temp-target.*" %MinerBat% 2>NUL 1>&2 || (
		ECHO Ensure --server --logfile --time --temp-target options added to the miners command line in this order.
		IF %ChatId% NEQ 0 CALL :telegram "false" "Ensure *--server* *--logfile* *--time* *--temp-target* options added to the miners command line in this order."
		>> %~n0.log ECHO [%Date%][%Time:~-11,8%] Ensure --server --logfile --time --temp-target options added to the miners command line in this order.
	)
...
FOR /F "tokens=5,6 delims=AMGPUSolWs上午下午/>#| " %%A IN ('findstr.exe /R /C:".*Sol/s.*" %Logfile%') DO (
	...
	IF !NumberOfGPUs! GEQ 2 IF "%%A" NEQ ":" (
		SET LastHashrate=%%A
		SET LastHashrate=!LastHashrate:~0,-2!
	)
