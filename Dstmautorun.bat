...
SET Server1BatCommand=%MinerProcess% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr187 --pass x --logfile --time --temp-target 80
SET Server2BatCommand=%MinerProcess% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr187 --pass x --logfile --time --temp-target 80
SET Server3BatCommand=%MinerProcess% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr187 --pass x --logfile --time --temp-target 80
SET Server4BatCommand=%MinerProcess% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr187 --pass x --logfile --time --temp-target 80
SET Server5BatCommand=%MinerProcess% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr187 --pass x --logfile --time --temp-target 80
...
IF EXIST "%Configfile%" (
	findstr.exe /C:"%Version%" %Configfile% >NUL && (
		FOR %%A IN (%~n0.bat) DO IF %%~ZA LSS 54171 EXIT
...
IF !ServerQueue! GEQ 6 >> %MinerBat% ECHO %MinerProcess% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr187 --pass x --logfile --time --temp-target 80
>> %MinerBat% ECHO EXIT
timeout.exe /T 5 /nobreak >NUL
...
IF NOT EXIST "%Logfile%" (
	...
	IF %ChatId% NEQ 0 powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot%Num%:%prt%-%rtp%%tpr%/sendMessage?parse_mode=markdown&chat_id=%ChatId%&text=*%RigName%:* %Logfile% is missing. Ensure *--logfile* option is added to the miners command line. Check permissions of this folder. This script requires permission to create files.')" 2>NUL 1>&2
	...
	findstr.exe /R /C:".*--server.*--logfile.*--time.*--temp-target.*" %MinerBat% 2>NUL 1>&2 || (
		ECHO Ensure --server --logfile --time --temp-target options added to the miners command line in correct order.
		IF %ChatId% NEQ 0 powershell.exe -command "(new-object net.webclient).DownloadString('https://api.telegram.org/bot%Num%:%prt%-%rtp%%tpr%/sendMessage?parse_mode=markdown&chat_id=%ChatId%&text=*%RigName%:* Ensure *--server* *--logfile* *--time* *--temp-target* options added to the miners command line in correct order.')" 2>NUL 1>&2
		>> %~n0.log ECHO [%Date%][%Time:~-11,8%] Ensure --server --logfile --time --temp-target options added to the miners command line in correct order.
	)
...
timeout.exe /T 5 /nobreak >NUL
FOR /F "tokens=2 delims=>#|" %%N IN ('findstr.exe /I /R %CriticalErrorsList% %MinerErrorsList% %MinerWarningsList% %InternetErrorsList% %Logfile%') DO SET LastError=%%N
IF !LastError! NEQ 0 (
	IF %EnableInternetConnectivityCheck% EQU 1 (
		ECHO !LastError!| findstr.exe /I /R %InternetErrorsList% 2>NUL 1>&2 && (
			FOR /F "tokens=2 delims=>#|" %%n IN ('findstr.exe /I /R %InternetErrorsList% %InternetErrorsCancel% %Logfile%') DO SET LastInternetError=%%n
...
FOR /F "tokens=5,6 delims=AMGPUSolWs上午下午/>#| " %%A IN ('findstr.exe /R /C:".*Sol/s.*" %Logfile%') DO (
	...
	IF !NumberOfGPUs! GEQ 2 IF NOT "%%A" == ":" (
		SET LastHashrate=%%A
		SET LastHashrate=!LastHashrate:~0,-2!
	)
...
IF %EnableLastShareDiffCheck% EQU 1 (
	...
	IF !PTOS1! LSS %Me2% (
		...
		FOR /F "tokens=3 delims=: " %%A IN ('findstr.exe /R /C:"GPU.*C.*Sol/s:.*\+" %Logfile%') DO SET LstShareMin=1%%A
