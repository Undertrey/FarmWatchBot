...
SET commandserver1=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr192 --pass x --logfile --time --temp-target 80
SET commandserver2=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr192 --pass x --logfile --time --temp-target 80
SET commandserver3=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr192 --pass x --logfile --time --temp-target 80
SET commandserver4=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr192 --pass x --logfile --time --temp-target 80
SET commandserver5=%minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr192 --pass x --logfile --time --temp-target 80
...
IF %queue% EQU 0 >> %bat% ECHO %minerpath% --server eu1-zcash.flypool.org --port 3333 --user t1S8HRoMoyhBhwXq6zY5vHwqhd9MHSiHWKv.fr192 --pass x --logfile --time --temp-target 80
>> %bat% ECHO EXIT
timeout.exe /T 3 /nobreak >NUL
START "%bat%" "%bat%" && (
...
IF NOT EXIST "%log%" (
	findstr.exe /R /C:".*--logfile.*" %bat% 2>NUL 1>&2 || (
		CALL :inform "false" "%log% is missing. Ensure *--logfile* option is added to the miners command line." "%log% is missing. Ensure --logfile option is added to the miners command line." "2"
		GOTO error
	)
	...
	findstr.exe /R /C:".*%minerprocess% --server.*--logfile.*--time.*--temp-target.*" %bat% 2>NUL 1>&2 || (
		CALL :inform "false" "Ensure *%minerpath% --server --logfile --time --temp-target* options added to the miners command line in this order." "Ensure %minerpath% --server --logfile --time --temp-target options added to the miners command line in this order." "2"
	)
...
FOR /L %%A IN (0,1,%totalgpucount%) DO (
	...
	IF "!tempdata!" NEQ "0" SET curtemp=!curtemp! G%%A !tempdata!C,
	IF !minhashrate! GEQ 99 GOTO passaveragecheck
