...
SET gpurestart=1
SET hashrate=0
SET minerprocess=nbminer.exe
SET minerpath=%minerprocess%
SET commandserver1=%minerpath% -o stratum+tcp://eu1.ethermine.org:4444 -a ethash -u 0x4a98909270621531dda26de63679c1c6fdcf32ea.d%ver:.=% --log-file %log%
SET commandserver2=%minerpath% -o stratum+tcp://eu1.ethermine.org:4444 -a ethash -u 0x4a98909270621531dda26de63679c1c6fdcf32ea.d%ver:.=% --log-file %log%
SET commandserver3=%minerpath% -o stratum+tcp://eu1.ethermine.org:4444 -a ethash -u 0x4a98909270621531dda26de63679c1c6fdcf32ea.d%ver:.=% --log-file %log%
SET commandserver4=%minerpath% -o stratum+tcp://eu1.ethermine.org:4444 -a ethash -u 0x4a98909270621531dda26de63679c1c6fdcf32ea.d%ver:.=% --log-file %log%
SET commandserver5=%minerpath% -o stratum+tcp://eu1.ethermine.org:4444 -a ethash -u 0x4a98909270621531dda26de63679c1c6fdcf32ea.d%ver:.=% --log-file %log%
SET ocprogram=0
SET profile=0
...
SET errorscancel=/C:".*Login succeeded.*"
SET criticalerrorslist=/C:".*CUDA.*Error.*" /C:".*exception, exit.*"
SET errorslist=/C:".*Logout.*"
SET interneterrorslist=/C:".*Failed.*connection.*"
...
IF %queue% EQU 0 >> %bat% ECHO %minerpath% -o stratum+tcp://eu1.ethermine.org:4444 -a ethash -u 0x4a98909270621531dda26de63679c1c6fdcf32ea.d%ver:.=% --log-file %log%
>> %bat% ECHO EXIT
timeout.exe /T 3 /nobreak >NUL
START "%bat%" "%bat%" && (
	CALL :inform "1" "false" "Miner was started." "Miner was started. Script v.%ver%." "Miner was started at %Time:~-11,8%"
	FOR /F "tokens=2,3 delims=/@:" %%a IN ('findstr.exe /R /C:".*%minerprocess%" %bat%') DO (
...
IF NOT EXIST "%log%" (
	findstr.exe /R /C:".*--log-file %log%.*" %bat% 2>NUL 1>&2 || (
		CALL :inform "1" "false" "%log% is missing. Ensure *--log-file %log%* option is added to the miners command line in *%config%* file." "%log% is missing. Ensure --log-file %log% option is added to the miners command line in %config% file." "2"
		PAUSE
		EXIT
	)
...
IF %firstrun% EQU 0 (
	timeout.exe /T %cputimeout% /nobreak >NUL
	FOR /F "delims=" %%A IN ('findstr.exe /R /C:".*INFO - \*|.*" %log%') DO SET /A gpucount+=1
...
timeout.exe /T %cputimeout% /nobreak >NUL
FOR /F "tokens=4 delims=|-. " %%A IN ('findstr.exe /R /C:".*INFO - |.*Total:.*M.*" %log%') DO (
	SET lasthashrate=%%A
...
FOR /L %%A IN (0,1,%gpus%) DO (
	...
	SET tempdata=null
	SET speeddata=null
	FOR /F "tokens=5,12 delims=|.- " %%a IN ('findstr.exe /R /C:".*INFO - .*%%A|.*|.* M|.*" %log%') DO (
		SET speeddata=%%a
		SET tempdata=%%b
